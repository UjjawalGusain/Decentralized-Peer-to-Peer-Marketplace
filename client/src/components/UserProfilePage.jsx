    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
        ...
        <div className="flex items-center gap-6 mb-10">
          <img
            src={user.profile.avatar || 'https://via.placeholder.com/100?text=No+Avatar'}
            alt={user.profile.name}
            className="w-24 h-24 rounded-full shadow-md object-cover"
          />
          <div>
            <h1 className="text-4xl font-bold text-slate-900">{user.profile.name}</h1>
        ...
        <section className="bg-white p-6 rounded-md shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-y-4 text-gray-800 text-lg">
            ...
            <div>
              ...
              <p>{user.profile.reputation?.totalTransactions ?? 0}</p>
            </div>
            <div>
              <span className="font-medium text-slate-700">Average Rating</span>
              <p>{user.profile.reputation?.averageRating?.toFixed(1) ?? 0}</p>
            </div>
          </div>
        </section>

        {/* Location Section */}
        {user.profile.location && user.profile.location.coordinates && (
          <section className="bg-white p-6 rounded-md shadow-sm">
            <SectionTitle>Location</SectionTitle>
            <p>
              Longitude: {user.profile.location.coordinates[0]}, Latitude:{" "}
